...
type PaymentService interface {
	CreatePayment(ctx context.Context, product *entity.Product, user *entity.User, transaction *entity.Transaction) (map[string]interface{}, error)
	HandleMidtransNotification(ctx context.Context, notif map[string]interface{}) error
	CreateTokenPayment(ctx context.Context, user *entity.User, product *entity.Product, transaction *entity.Transaction) error
...
func NewPaymentService(cfg *config.Config, userRepository repository.UserRepository, transactionRepository repository.TransactionRepository, productRepository repository.ProductRepository) PaymentService {
	return &paymentService{cfg, userRepository, transactionRepository, productRepository}
}

func (s *paymentService) CreatePayment(ctx context.Context, product *entity.Product, user *entity.User, transaction *entity.Transaction) (map[string]interface{}, error) {
	...
	request := &snap.Request{
		TransactionDetails: midtrans.TransactionDetails{
			...
			GrossAmt: int64(transaction.TransactionAmount),
		},
	...
	if err := s.sendPaymentConfirmationEmail(user.Email, product.ProductName, snapResponse.RedirectURL); err != nil {
...
func (s *paymentService) CreateTokenPayment(ctx context.Context, user *entity.User, product *entity.Product, transaction *entity.Transaction) error {
	...
	var ReplacerEmail = struct {
		...
		Token: transaction.VerificationToken,
	}
